...
const AdminDashboard = () => {
    ...
    const [deleteTarget, setDeleteTarget] = useState(null); // { type: 'post'|'book', id, title }
    ...
    const fetchPendingBooks = async () => {
        try {
            const res = await api.get('/books/admin/pending/');
    ...
    const fetchApprovedBooks = async () => {
        try {
            const res = await api.get('/books/admin/approved/');
            setApprovedBooks(res.data || []);
        } catch (err) { console.error(err); }
    };
    ...
    const fetchAllPosts = async () => {
        try {
            const res = await api.get('/posts?page=1&page_size=50');
    ...
    const handleApproveBook = async (bookId) => {
        setBookLoading(true);
        try {
            await api.post(`/books/admin/${bookId}/approve/`);
            showSuccess('Book approved!');
            fetchPendingBooks();
            fetchApprovedBooks();
        } catch (err) {
    ...
    const handleRejectBook = async (bookId) => {
        setBookLoading(true);
        try {
            await api.post(`/books/admin/${bookId}/reject/`);
            showSuccess('Book rejected.');
            fetchPendingBooks();
        } catch (err) {
            setError(err.response?.data?.error || 'Failed to reject book');
        } finally { setBookLoading(false); }
    };
    ...
    const handleApproveReset = async (requestId) => {
        ...
        try {
            await api.post(`/admin/password-reset-requests/${requestId}/process`, {
                action: 'approve',
            });
            setResetDialog(false);
            setSelectedReset(null);
            showSuccess('Password reset approved! User can now reset their password.');
            fetchResetRequests();
        } catch (err) {
    ...
    const handleRejectReset = async (requestId) => {
        ...
        try {
            ...
            setError(err.response?.data?.error || 'Failed to reject reset');
        } finally { setResetLoading(false); }
    };
    ...
    return (
        <Layout>
                ...
                <Box sx={{ py: 4 }}>
                    ...
                    <Grid container spacing={3} sx={{ mb: 4 }}>
                        {[
                            ...
                            { label: 'Pending Resets', value: pendingResets.length, color: '#d63031', icon: <LockResetIcon /> },
                            ...
                            label={
                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                    ...
                                    {pendingResets.length > 0 && (
                                        <Chip label={pendingResets.length} size="small" color="error" sx={{ height: 18, fontSize: 10 }} />
                                    )}
                                </Box>
                            }
                                        ...
                                        <Card sx={{ borderRadius: 3, border: '1px solid', borderColor: 'warning.light' }}>
                                            <CardContent>
                                                <Box sx={{ display: 'flex', gap: 2 }}>
                                                    {/* Fixed: separate img and div, no conditional component */}
                                                    {book.cover_image ? (
                                                        <Box
                                                            component="img"
                                                            src={book.cover_image}
                                                            alt={book.title}
                                                            sx={{ width: 70, height: 100, borderRadius: 2, objectFit: 'cover', flexShrink: 0 }}
                                                        />
                                                    ) : (
                                                        <Box sx={{
                                                            width: 70, height: 100, borderRadius: 2, flexShrink: 0,
                                                    ...
                                                    <Box sx={{ flexGrow: 1, minWidth: 0 }}>
                                                        ...
                                                        <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mb: 0.5 }}>
                                                            by @{book.author_name || book.author_username || `Writer #${book.author_id}`}
                                                        </Typography>
                                            ...
                                            <Box sx={{ display: 'flex', gap: 1, p: 2 }}>
                                                <Button variant="contained" color="success" size="small"
                                                    ...
                                                    disabled={bookLoading} sx={{ flex: 1 }}>
                                                    Approve
                                                </Button>
                                        ...
                                        <Card sx={{ borderRadius: 3, height: '100%' }}>
                                            <CardContent>
                                                <Box sx={{ display: 'flex', gap: 2, mb: 1.5 }}>
                                                    {book.cover_image ? (
                                                        <Box component="img" src={book.cover_image} alt={book.title}
                                                            sx={{ width: 50, height: 70, borderRadius: 1, objectFit: 'cover', flexShrink: 0 }} />
                                                    ) : (
                                                        <Box sx={{
                                                            width: 50, height: 70, borderRadius: 1, flexShrink: 0,
                    ...
                    {activeTab === 2 && (
                        <Box>
                            <Typography variant="h6" fontWeight={600} sx={{ mb: 2 }}>
                                🔴 Pending Requests ({pendingResets.length})
                            </Typography>
                            ...
                            {processedResets.length > 0 && (
                                <>
                                    <Typography variant="h6" fontWeight={600} sx={{ mb: 2 }}>
                                        ✅ Processed ({processedResets.length})
                                    </Typography>
                                                ...
                                                <Card sx={{ borderRadius: 3, opacity: 0.8 }}>
                                                    <CardContent>
                                                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                                                            ...
                                                            <Chip
                                                                label={req.status === 'processed' ? 'Approved' : 'Rejected'}
                                                                color={req.status === 'processed' ? 'success' : 'error'}
                                                                size="small"
                                                            />
                                                        </Box>
                                                    </CardContent>
                    ...
                    {/* ── Tab 3: All Posts (admin can delete) ── */}
                    {activeTab === 3 && (
                        allPosts.length === 0 ? (
                            <Paper sx={{ p: 4, textAlign: 'center', borderRadius: 3 }}>
                                            ...
                                            <CardContent>
                                                <Box sx={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 2 }}>
                                                    <Box sx={{ display: 'flex', gap: 1.5, flexGrow: 1, minWidth: 0 }}>
                                                        <Avatar src={post.profile_picture} sx={{ width: 36, height: 36, flexShrink: 0 }}>
                                                            {(post.username || post.author_name)?.charAt(0).toUpperCase()}
                                                        </Avatar>
                                                        <Box sx={{ minWidth: 0 }}>
                                                            <Typography variant="subtitle2" fontWeight={600}>
                                                                @{post.username || post.author_name || `User #${post.author_id}`}
                                                            </Typography>
                                                            <Typography variant="caption" color="text.secondary">
                                                            ...
                                                            {post.image && (
                                                                <Box component="img" src={post.image} alt="post"
                                                                    sx={{ mt: 1, maxHeight: 80, borderRadius: 1, display: 'block' }} />
                                                            )}
                                                        </Box>
                                                    </Box>
                                                    <Button
                                                        ...
                                                        onClick={() => confirmDelete('post', post.id, post.content?.substring(0, 30))}
                                                        sx={{ flexShrink: 0 }}
                                                    >
                                                        Delete
                                                    </Button>
            ...
            <Dialog open={deleteDialog} onClose={() => setDeleteDialog(false)} maxWidth="xs" fullWidth>
                <DialogTitle>
                    <Typography variant="h6" fontWeight={700} color="error">Delete Post?</Typography>
                </DialogTitle>
                <DialogContent>
                    <Typography variant="body2" color="text.secondary">
                        This will permanently delete the post: <strong>"{deleteTarget?.title}..."</strong>
                    </Typography>
                    <Typography variant="body2" color="error" sx={{ mt: 1 }}>This action cannot be undone.</Typography>
                </DialogContent>
                <DialogActions>
                    <Button onClick={() => setDeleteDialog(false)}>Cancel</Button>
                    <Button variant="contained" color="error" onClick={handleDeletePost} disabled={postsLoading}>
                        {postsLoading ? <CircularProgress size={20} /> : 'Delete'}
                    </Button>
                </DialogActions>
            </Dialog>
        </Layout>
